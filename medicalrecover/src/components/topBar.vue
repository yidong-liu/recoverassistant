<template>
  <div class="topBar">
    ...
    <div class="right">
      <div @click="showHoverBlock" class="myInfo">
        <span>用户名：</span>
        <span style="padding-right: 5px;">{{ userName }}</span>
        <i class="iconfont icon-shezhi"></i>
      </div>
      <div :class="{ hoverBlockShow: isShow, hoverBlock: true }">
        <span class="updatePassword" @click="goUpdatePasswprdPage">修改密码</span>
        <span class="logOut" @click="myLogOut">退出登陆</span>
      </div>
    </div>
  </div>
  <updatePassword
    v-if="showUpdatePass"
    :getHideInformation="getHideInformation"
    :userName="userName"
  ></updatePassword>
</template>
...
let mode = ref(true)
let isShow = ref(true)
let showUpdatePass = ref(false)
const changeSize = () => {
  mode.value = !mode.value
}
watch(mode, (mode, preMode) => {
  appContext.config.globalProperties.$mitt.emit('changeSize', mode)
})
const showHoverBlock = () => {
  isShow.value = !isShow.value
}
const goUpdatePasswprdPage = () => {
  isShow.value = true
  showUpdatePass.value = true
}
const getHideInformation = (hideInfo) => {
  showUpdatePass.value = hideInfo
}
...
const myLogOut = async () => {
  try {
    ...
    let result = await logOut(role, userName)
    //console.log(result)
